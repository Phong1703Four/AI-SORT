// ======================== EMOJI MAPS ========================
// Mỗi loại rác sẽ có emoji riêng biệt, không trùng nhau, không lộ đáp án

const organicEmojis: Record<string, string> = {
  'chuối': '🍌', 'banana': '🍌',
  'táo': '🍎', 'apple': '🍎',
  'cam': '🍊', 'orange': '🍊',
  'xoài': '🥭', 'mango': '🥭',
...
const recycleEmojis: Record<string, string> = {
...
const inorganicEmojis: Record<string, string> = {
  'nilon': '🛍️', 'nylon': '🛍️',
  'nhựa xốp': '📦', 'styrofoam': '📦',
  'cao su': '🔘', 'rubber': '🔘',
...
const hazardousEmojis: Record<string, string> = {
  'AA': '🔋', 'AAA': '🪫',
  'lithium': '⚡', 'lead': '🏭',
  'chì': '🏭',
...
const generateWasteItems = (): WasteItem[] => {
  let items: WasteItem[] = [];
  let currentId = 1;
  ...
  // Trộn ngẫu nhiên
  return items.sort(() => Math.random() - 0.5).map((item, idx) => ({ ...item, id: idx + 1 }));
};

export const WASTE_ITEMS = generateWasteItems();
